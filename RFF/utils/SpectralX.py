...
class SpectralX(nn.Module):
    def __init__(self, num_classes, nf=32, adaptive_size=50, use_asb=True, **kwargs):
        super(SpectralX, self).__init__()
        self.nf = nf
        self.use_asb = use_asb
        self.cls_nf = self.nf * 32
        self.emb = XceptionBlock(2, nf, **kwargs)
        self.fdr = nn.Sequential(
            nn.AdaptiveAvgPool1d(adaptive_size),
            Conv1x1(self.cls_nf, self.cls_nf // 2), 
            Conv1x1(self.cls_nf // 2, self.cls_nf // 4)
        )
        self.cls = nn.Sequential(
            Conv1x1(self.cls_nf // 4, num_classes),
            GAP1d(1)
        )
    ...
    def forward(self, x):
        ...
        if self.use_asb:
            x = self.asb(x)
        emb_out = self.fdr(x) 
        cls_out = self.cls(emb_out)
        return emb_out, cls_out
